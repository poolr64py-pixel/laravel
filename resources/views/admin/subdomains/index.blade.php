...
@section('content')
    <div class="page-header">
        ...
        <ul class="breadcrumbs">
            ...
            <li class="nav-item">
                <a
                    href="#">{{ empty(request()->input('type')) ? __('All') : __(ucfirst(request()->input('type'))) }}</a>
            </li>
        </ul>
    </div>
    <div class="row">
                                    ...
                                    <table class="table table-striped mt-3">
                                        ...
                                        <tbody>
                                            @foreach ($subdomains as $subdomain)
                                                <tr>
                                                    <td>{{ $subdomain->user->username }}</td>
                                                    <td>{{ $subdomain->website?->website_name ?? 'N/A' }}</td>
                                                    <td>
                                                        <a target="_blank"
                                                            href="//{{ strtolower($subdomain->requested_subdomain) }}.{{ env('WEBSITE_HOST') }}">{{ strtolower($subdomain->requested_subdomain) }}.{{ env('WEBSITE_HOST') }}</a>
                                                    </td>
                                                    {{-- <td>
                                                        <span>{{  $subdomain->current_subdomain }}.{{ env('WEBSITE_HOST') }}</span>
                                                        
                                                    </td> --}}
                                                    <td>{{ is_null($subdomain->current_subdomain) || $subdomain->current_subdomain == 0 ? '-' : $subdomain->current_subdomain . '.' . env('WEBSITE_HOST') }}
                                                    </td>
                                                    <td>
                                                        <form id="statusForm{{ $subdomain->id }}"
                                                            ...
                                                            <input type="hidden" name="id"
                                                                value="{{ $subdomain->id }}">
                                                            <select
                                                                class="max-w-130 form-control form-control-sm
                                                            @if ($subdomain->status == 0) bg-warning text-white
                                                            @elseif($subdomain->status == 1)
                                                                bg-success text-white 
                                                                 @elseif($subdomain->status == 2)
                                                                bg-danger text-white 
                                                                 @elseif($subdomain->status == 3)
                                                                bg-dark text-white @endif
                                                                "
                                                                name="status"
                                                                onchange="document.getElementById('statusForm{{ $subdomain->id }}').submit();">
                                                                <option value="0"
                                                                    {{ $subdomain->status == 0 ? 'selected' : '' }}>
                                                                    {{ __('Pending') }}
                                                                </option>
                                                                <option value="1"
                                                                    {{ $subdomain->status == 1 ? 'selected' : '' }}>
                                                                    {{ __('Connected') }}
                                                                </option>
                                                                <option value="2"
                                                                    {{ $subdomain->status == 2 ? 'selected' : '' }}>
                                                                    {{ __('Rejected') }}
                                                                </option>
                                                                <option value="3"
                                                                    {{ $subdomain->status == 3 ? 'selected' : '' }}>
                                                                    {{ __('Revomed') }}
                                                                </option>
                                                            </select>
                                                        </form>
                            ...
                            <div class="modal-body">
                                <form id="ajaxEditForm" class="" action="{{ route('admin.custom-domain.mail') }}"
                                    ...
                                    <div class="form-group">
                                        <label for="">{{ __('Email Address') }} <span

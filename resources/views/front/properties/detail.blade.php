...
<script>
function sendWhatsAppProperty(form) {
    const name = form.querySelector('input[name="name"]').value;
    const email = form.querySelector('input[name="email"]').value;
    const phone = form.querySelector('input[name="phone"]').value;
    const message = form.querySelector('textarea[name="message"]').value;
    const title = document.querySelector('h1')?.textContent || 'Imóvel';
    const url = window.location.href;
    const msg = `*Interesse em Imóvel*\n\n*Imóvel:* ${title}\n*Link:* ${url}\n\n*Nome:* ${name}\n*Email:* ${email}\n*Telefone:* ${phone}\n*Mensagem:* ${message}`;
...
@section('content')
    ...
    <section class="property-detail ptb-90">
        <div class="container">
            <div class="row">
                <div class="col-lg-8">
...
@endif
                    ...
                    <div class="property-info mb-4 p-4 bg-white rounded shadow-sm">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <div>
                                <span class="badge bg-primary mb-2">{{ ucfirst($property->purpose) }}</span>
                                <span class="badge bg-secondary mb-2 ms-2">{{ ucfirst($property->type) }}</span>
                                @if($property->featured)
                                    <span class="badge bg-warning text-dark mb-2 ms-2">⭐ Destaque</span>
                                @endif
                            </div>
                            ...
                            <div class="row text-center">
                                @if($property->beds)
                                    <div class="col-3">
                                        ...
                                        <p class="mb-0"><strong>{{ $property->beds }}</strong></p>
                                        <small>Quartos</small>
                                    </div>
                                @endif
                                @if($property->bath)
                                    <div class="col-3">
                                        ...
                                        <p class="mb-0"><strong>{{ $property->bath }}</strong></p>
                                ...
                                @if($property->area)
                                    <div class="col-3">
                                        ...
                                        <p class="mb-0"><strong>{{ $property->area }}</strong></p>
                                        <small>m²</small>
                                    </div>
                                @endif
                                <div class="col-3">
                                    <i class="fas fa-hashtag fa-2x text-primary mb-2"></i>
                                    <p class="mb-0"><strong>#{{ $property->id }}</strong></p>
                                    <small>Código</small>
                                </div>
                            </div>
                        </div>
...
@endif
                    @if($property->latitude && $property->longitude)
                        <div class="property-map mb-4 p-4 bg-white rounded shadow-sm">
                            <h4 class="mb-3">Localização</h4>
                            <div class="ratio ratio-16x9">
                                <iframe
                                    src="https://maps.google.com/maps?q={{ $property->latitude }},{{ $property->longitude }}&hl=pt&z=14&output=embed"
                                    style="border:0;"
                                    allowfullscreen=""
                                    loading="lazy">
                                </iframe>
                ...
                <div class="col-lg-4">
                    <div class="contact-card card mb-4 shadow-sm">
                        <div class="card-body">
                            <h5 class="card-title mb-3">Interessado neste imóvel?</h5>
                            <form action="#" method="POST" onsubmit="event.preventDefault(); sendWhatsAppProperty(this); return false;">
                                @csrf
            ...
            @if($related_properties && $related_properties->count() > 0)
                <div class="related-properties mt-5">
                    <h3 class="mb-4">Imóveis Relacionados</h3>
                    <div class="row">
                        @foreach($related_properties as $relatedProperty)
                            @if($relatedProperty->current_content)
                            <div class="col-lg-4 col-md-6 mb-4">
                                <div class="card h-100 shadow-sm">
                                    <div style="height:200px;overflow:hidden;">
                                        <a href="{{ route('front.property.detail', $relatedProperty->current_content?->slug) }}">
                                            <img src="{{ asset('assets/img/property/featureds/'.$relatedProperty->featured_image) }}"
                                                 class="card-img-top" 
                                    ...
                                    <div class="card-body">
                                        ...
                                        @if($relatedProperty->city_name)
                                            <p class="text-muted small">
                                                <i class="fas fa-map-marker-alt"></i> {{ $relatedProperty->city_name }}
                                            </p>
                                        @endif
                                    </div>
                                </div>
...
function changeMainImage(imageSrc, thumbElement) {
    ...
    // Remover borda de todas as miniaturas
    document.querySelectorAll('.gallery-thumb').forEach(function(thumb) {
        thumb.style.border = '2px solid transparent';
    });
    
    // Adicionar borda na miniatura clicada
    thumbElement.style.border = '2px solid #007bff';
}
</script>
<script>
function sendWhatsAppProperty(form) {

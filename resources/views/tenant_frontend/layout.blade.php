@php
    ...
    $cookieAlertInfo = $cookieAlertInfo ?? null;
    $popupInfos = $popupInfos ?? [];
    $footerSectionStatus = $footerSectionStatus ?? null;
    $tenant = $tenant ?? getUser();
    $currentLanguageInfo = $currentLanguageInfo ?? \App\Models\User\Language::where('is_default', 1)->where('user_id', $tenant->id ?? 0)->first();
$menuInfos = $menuInfos ?? [];
@endphp
<!DOCTYPE html>

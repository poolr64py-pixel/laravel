                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-6 offset-lg-3">
                            <form id="logoForm" action="{{ route('user.footer.update_logo') }}" method="POST"
                                enctype="multipart/form-data">
                                @csrf
                                <div class="form-group">
                                    <label for="">{{ __('Logo') }} <span
                                            class="text-danger">{{ '*' }}</span> </label>
                                    <br>
                                    <div class="thumb-preview">
                                        @if (!empty($data->footer_logo))
                                            <img src="{{ asset(\App\Constants\Constant::WEBSITE_FOOTER_LOGO . '/' . $data->footer_logo) }}"
                                                alt="logo" class="uploaded-img">
                                        @else
        ...
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-6 offset-lg-3">
                            <form id="imageForm" action="{{ route('user.footer.update_bg_image') }}" method="POST"
                                enctype="multipart/form-data">
                                @csrf
                                <div class="form-group">
                                    ...
                                    <br>
                                    <div class="thumb-preview">
                                        @if (!empty($data->footer_bg_img))
                                            <img src="{{ asset(\App\Constants\Constant::WEBSITE_FOOTER_LOGO . '/' . $data->footer_bg_img) }}"
                                    ...
                                    @if ($errors->has('footer_background_image'))
                                        <p class="mt-2 mb-0 text-danger">{{ $errors->first('footer_background_image') }}
                                        </p>
                                    @endif
                                </div>
                            </form>

    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="ajaxEditForm" class="modal-form" action="{{ route('user.advertise.update_advertisement') }}"
                    ...
                    <div class="form-group">
                        <label for="">{{ __('Advertisement Type')   }} <span class="text-danger">{{ '*' }}</span></label>
                    ...
                    <div class="form-group">
                        <label for="">{{ __('Advertisement Resolution')   }} <span class="text-danger">{{ '*' }}</span></label>
                        <select name="resolution_type" class="form-control" id="inresolution_type">
                            <option disabled>{{ __('Select a Resolution') }}
                            </option>
                        ...
                        <div class="col-lg-12">
                            <div class="form-group d-none" id="edit-image-input">
                                <div class="col-12 mb-2">
                                    <label for="image"><strong>{{ __('Image')   }}</strong> <span class="text-danger">{{ '*' }}</span></label>
                                </div>
                                <div class="col-md-12 showImage mb-3">
                                    <img src="" alt="..." id="in_image" class="img-thumbnail">
                                </div>
                    ...
                    <div class="form-group d-none" id="edit-url-input">
                        <label for="">{{ __('Redirect URL') }} <span class="text-danger">{{ '*' }}</span></label>
                        <input type="url" class="form-control" name="url"
                            placeholder=" {{ __('Enter redirect url') }}" id="inurl">
                        <p id="editErr_url" class="mt-2 mb-0 text-danger em"></p>
                    </div>

                    <div class="form-group d-none" id="edit-slot-input">
                        <label for="">{{ __('Ad Slot')   }} <span class="text-danger">{{ '*' }}</span> </label>
                        <input type="text" class="form-control" name="slot"
                            placeholder="{{ __('Enter ad slot') }}" id="inslot">

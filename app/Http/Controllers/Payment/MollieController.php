...
use App\Services\PaymentGateway\MollieService;
...
class MollieController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->mollieService->setCredentials(true);
    }

    public function paymentProcess(Request $request, $_amount, $_success_url, $_cancel_url, $_title, $webCuryency)
    {

        Session::put('request', $request->all());

        $validCurrency = $this->mollieService->checkCurrency($webCuryency, array('AED', 'AUD', 'BGN', 'BRL', 'CAD', 'CHF', 'CZK', 'DKK', 'EUR', 'GBP', 'HKD', 'HRK', 'HUF', 'ILS', 'ISK', 'JPY', 'MXN', 'MYR', 'NOK', 'NZD', 'PHP', 'PLN', 'RON', 'RUB', 'SEK', 'SGD', 'THB', 'TWD', 'USD', 'ZAR'));

        if (!$validCurrency) {
            return redirect()->back()->with('error', __('Currency is not valid for this payment gateway') . '.')->withInput($request->all());
        }
        return $this->mollieService->makePayment($_amount, $_success_url, $_title, $webCuryency);
    }

    public function successPayment(Request $request)
    {
        $cancel_url = route('membership.mollie.cancel');
        $verifyPayment = $this->mollieService->verifyPayment($request->payment_id, $cancel_url);
        ...
        if ($verifyPayment->status == 'paid') {
            ...
            $transaction_details = json_encode($verifyPayment);

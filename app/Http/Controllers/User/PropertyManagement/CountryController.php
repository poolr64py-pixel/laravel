...
class CountryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('user.property-management.country.index', $information);
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($languages as $lan) {
            $rules[$lan->code . '_name'] =
                [
                    'required',
                    Rule::unique('user_country_contents', 'name')->where('user_id', $tenantId)
    ...
    public function update(Request $request)
    {
        ...
        foreach ($languages as $lan) {
            $rules[$lan->code . '_name'] =
                [
                    'required',
                    Rule::unique('user_country_contents', 'name')->ignore($request->id, 'country_id')->where('user_id', $tenantId)
                ];
            $message[$lan->code . '_name.required'] = __('The name field is required for') . ' ' . $lan->name . ' '  . __('language');
            $message[$lan->code . '_name.unique'] = __('The name field must be unique for') . ' ' . $lan->name . ' ' . __('language');
        }

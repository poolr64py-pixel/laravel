...
//         $information['language'] = $language;
    ...
    {
        \Log::info("SEO PAGE ACCESSED");
        
        // Se não tiver language, redireciona para inglês
        if (!$request->has('language') || empty($request->language)) {
            return redirect()->route('admin.seo', ['language' => 'en']);
        }
    ...
    public function updateSEO(Request $request)
    {
        ...
        if (!$language) {
            \Session::flash("error", "Please select a valid language");
            return back();
        }

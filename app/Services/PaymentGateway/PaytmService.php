...
class PaytmService extends PaymentService
{
  private $paydata, $paytm_txn_url;
  public function setCredentials($isAdmin = true, $tenantId = null)
  {
    $data = $this->getGatewayData('paytm', $isAdmin, $tenantId);


    if (!$data) {
      throw new \Exception('Paytm credentials not found.');
    }

    $this->paydata = $data->convertAutoData();
    if ($this->paydata['environment'] == 'local') {
    ...
    // return $this->renderPaytmView($paytmData['paytm_txn_url'], $paytmData['paramList'], $paytmData['checkSum']);
  }

  public function renderPaytmView($_amount, $_item_number,  $_callback_url)
  {
    // dd($_amount, $_item_number, $_callback_url);
    $data_for_request = $this->handlePaytmRequest($_item_number, $_amount, $_callback_url);
    $paramList = $data_for_request['paramList'];
    $checkSum = $data_for_request['checkSum'];
    $paytm_txn_url = $this->paytm_txn_url;

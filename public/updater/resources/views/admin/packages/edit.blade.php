                    ...
                    <div class="row">
                        <div class="col-lg-6 offset-lg-3">
                            <form id="ajaxForm" class="" action="{{ route('admin.package.update') }}" method="post"
                                    ...
                                    @if ($hasAIContent || $hasAIImage)
                                        <div class="mt-2">
                                            <small class="text-warning">
                                                <strong>{{ __('Powered by Advanced AI') . ':' }}</strong>
                                            </small>
                                            <br>
                                            @if ($hasAIContent)
                                                <small class="text-warning">
                                                    • {{ __('Content Generation powered by Google Gemini') }}
                                                </small>
                                                <br>
                                            @endif
                                            @if ($hasAIImage)
                                ...
                                {{-- Conditional Inputs --}}
                                @php
                                    $hasPermission = fn($feat) => is_array($permissions) &&
                                        in_array($feat, $permissions);
                                    $isVisible = fn($feat) => in_array($feat, $features ?? []) && $hasPermission($feat);
                                @endphp
                                <!-- AI Content Tokens -->


                                @if (in_array('AI Content Generation', $features ?? []))
                                    <div class="form-group" id="ai_content_input"
                                        style="{{ $isVisible('AI Content Generation') ? '' : 'display: none;' }}">
                                        ...
                                        <input id="ai_tokens" type="number" class="form-control" name="ai_tokens"
                                            value="{{ $package->ai_tokens ?? '' }}"
                                            placeholder="{{ __('Enter number of AI content generation tokens') }}">

                                        <p id="errai_tokens" class="mb-0 text-danger em"></p>

                                        <small class="text-warning d-block mt-2">
                                            <i class="fas fa-info-circle"></i> {{ __('Token Information') . ':' }}<br>
                                            • {{ __('1 token = approximately 4 characters') }}<br>
                                            • {{ __('Enter 999999 for unlimited tokens') }}<br>
                                            • {{ __('Recommended: 10,000 - 50,000 tokens per package') }}
                                        </small>
                                    </div>
                                @endif

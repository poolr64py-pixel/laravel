    ...
    <div class="modal-dialog modal-dialog-centered " role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">{{ __('Add Language') }}
                </h5>
            ...
            <div class="modal-body">
                ...
                <form id="ajaxForm" class="" action="{{ route('user.language.store') }}" method="POST">
                    ...
                    <div class="form-group">
                        <label for="">{{ __('Code') }} <span class="text-danger">{{ '*' }}</span>
                        </label>
                    ...
                    <div class="form-group">
                        ...
                        <select name="direction" class="form-control">
                            <option value="" selected disabled>
                                {{ $keywords['Select a Direction'] ?? __('Select a Direction') }}</option>
                            <option value="0">{{ __('LTR (Left to Right)') }}
                            </option>
                            <option value="1">{{ __('RTL (Right to Left)') }}
                            </option>
                        </select>

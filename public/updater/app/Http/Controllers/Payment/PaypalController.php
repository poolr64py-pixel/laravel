...
class PaypalController extends Controller
{
    ...
    {
        try {
            $this->payPalService->setCredentials(true);
            ...
            return redirect($cancelUrl)->with('error', __('Something went wrong!'));
        } catch (\Exception $e) {
    ...
    public function successPayment(Request $request)
    {
        ...
        try {
            ...
            if (!isset($response['error']) && isset($response['status']) && $response['status'] == 'COMPLETED') {
                ...
                $requestData['payment_method'] = 'Paypal';
                ...
                return redirect()->route('success.page')->with('success', 'Payment completed.');
            }

            return redirect($cancel_url)->with(
                'error',
                'Payment failed.'
            );
        } catch (\Exception $e) {

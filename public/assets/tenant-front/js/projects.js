...
function getData(currentURL, page) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $('.projects-area .row').html();
      $('.projects-area .row').html(data.projectContents);

      var project_contents = data.projects;
      projects = project_contents.data;
      var mapId = $(".btn[data-bs-target='#mapModal']").is(":visible") ? "modal-main-map" : "project-map";
      mapInitialize(mapId, project_contents.data);
...
function getCities(e) {
  ...
  let cityUrl = tenantBaseURL + '/project/cities'
  $.ajax({
    type: 'GET',
    url: cityUrl,
    data: {
...
$(function () {
  ...
  $(".project_country").on('change', function (e) {
    ...
    let stateUrl = tenantBaseURL + '/project-state-cities'
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.states.length > 0) {
          ...
          $.ajax({
            type: 'GET',
            url: tenantBaseURL + '/project-cities',
            data: {
